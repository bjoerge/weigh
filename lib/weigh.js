...
module.exports = function weigh (packages, options) {
  ...
  var installProgress = installPackages(packages, {prefix: options.prefix}, function (err, installedPackages) {
    ...
    var bundleStream = options.bundler.bundle(packages, {
      env: options.env,
    ...
    const streams = [bundleStream, minifiedStream, gzipStream].filter(Boolean)
    const finalStream = streams.pop()
    streams.forEach(function (stream) {
      stream.on('error', throwError)
      stream.pipe(devnull())
    })
    finalStream.pipe(options.output ? process.stdout : devnull())
  })
